...
folder = os.path.join(projectdir,'2.gh_userinfo_dl','1.data','user_datas')
...
def create_frames(file_list):
    return [json_to_df(f,folder) for f in tqdm(file_list)]

frames1 = Parallel(n_jobs=8)(delayed(create_frames)(file_list1) for f in file_list1)
...
print('Pickling the dataframe to user_datas_df2.pkl')
pickle.dump(df1, open(os.path.join(projectdir,subfolderdir,picklesdir,'user_datas_df2.pkl'),'wb'))
print('\nPickling finished!')
